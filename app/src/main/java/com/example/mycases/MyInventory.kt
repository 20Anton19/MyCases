...
fun MyInventory(weaponViewModel: WeaponViewModel, onClick: () -> Unit) {
    ...
    //val inventoryList = weaponViewModel.getInventoryVM()
        ...
        LazyColumn(
                ...
                LazyRow(
                    ...
                    items(4) { index ->
                        val resourceId = ResourceGenerator.getResourceId(
                            context,
                            weaponList[(firstIndex*4)+index].name,
                            weaponList[(firstIndex*4)+index].skin)
                        Image(
            ...
            item {
                LazyRow(
                    ...
                    items(lastRowLen) { index ->
                        val resourceId = ResourceGenerator.getResourceId(
                            context,
                            weaponList[(rowsAmount*4)+index].name,
                            weaponList[(rowsAmount*4)+index].skin)
                        Image(
                            painter = painterResource(id = resourceId),
                            contentDescription = "mainbg",
                            modifier = Modifier
